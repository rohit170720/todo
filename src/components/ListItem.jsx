...
function ListItem({ title, description, date, onDelete, onEdit, onCheck, complete, task, dayDiff }) {
  ...
  console.log(diff)
  return (
    <div className={`w-11/12 shadow-2xl flex items-center justify-between p-4 border-b rounded-xl my-4 ${diff>3?'bg-green-600':(diff<3 && diff>0)?'bg-orange-300':'bg-red-600'}`}>
